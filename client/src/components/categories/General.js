...
const General = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://api.mediastack.com/v1/news?access_key=${API_KEY}&limit=100&categories=general&languages=en&sort=published_desc`
      )
      .then((data) =>
        setGeneral(data.data.data.filter((item) => item.image).slice(0, 6))
      )
      ...
      <div>
        <div key={i}>
          <div className={i === 0 ? "generalwrapperActive" : "generalwrapper"}>
            <img
              className={i === 0 ? "imgActive" : "generalimg"}
              src={image}
              alt={title}
              width="200px"
              height="250px"
              ...
              <div className="generalAuthorContainer">
                ...
                >
                  <p
                    className={
                      (i === 0 && contextObj.store === "light"
                        ? "generalAuthorWordActive"
                        : "generalAuthorWordActiveDark",
                      contextObj.store === "light"
                        ? "generalAuthorWord"
                        : "generalAuthorWordDark")
                    }
                  >
                    Author:{" "}
                  </p>
  ...
  return (
    <div>
      ...
      <div className="generalFather">{generalNews}</div>
    </div>
  );
};
