...
const NewsCollection = (props) => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://api.mediastack.com/v1/news?access_key=${API_KEY}&limit=100&categories=${category}&languages=en&sort=published_desc`
      )
          ...
          <div className="otherredirectFather">
            ...
            <p
              className={
                contextObj.store === "light"
                  ? "redirectDescription"
                  : "redirectDescriptionDark"
              }
            >
              {description}
            </p>
            <div className="down">
              <div className="authorFather">
                <p
                  className={
                    contextObj.store === "light"
                      ? "redirectAuthorWord"
                      : "redirectAuthorWordDark"
                  }
                >
                  Author:{" "}
                </p>{" "}
...
const [newsarray, setNewsarray] = useState([]);
  ...
  return (
    <div
      className={
        contextObj.store === "light"
          ? "redirectContainerFather"
          : "redirectContainerFatherDark"
      }
    >
      <LatestNews />
